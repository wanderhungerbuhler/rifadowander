        ...
        <div className="choosenumbers">
          {loadData?.map((number) => (
            <Btn
              key={number?.id}
              style={{
                border: number?.status_payment === "pending" ? "" : selectedItems?.includes(number?.id as never) ? "2px solid #723EB5" : "",
                background: number?.status_payment === "pending" ? "#1E1E1E" : "",
                color: number?.status_payment === "pending" ? "#323232" : "",
                textDecoration: number?.status_payment === "pending" ? "line-through" : ""
              }}
